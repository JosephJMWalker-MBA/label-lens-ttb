...
describe("bounded Brand fixed local-contrast governance", () => {
  ...
  it("leaves production OCR and the PR 195 Brand selector baseline unchanged", () => {
    expect(
      Object.fromEntries(
        Object.keys(HASHES_AT_MERGED_PR_198).map((filePath) => [filePath, sha256File(filePath)]),
      ),
    ).toEqual(HASHES_AT_MERGED_PR_198);
  });
});
