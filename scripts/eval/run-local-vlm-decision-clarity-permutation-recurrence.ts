...
import {
  ...
  writeDecisionClarityPermutationRecurrenceDiagnosticFiles,
} from "../../src/fixtures/eval/vision-observer/local-vlm/decision-clarity-permutation-recurrence-diagnostic.ts";
...
async function main() {
  ...
  if (!config.ok) {
    console.log(
      "SKIP: local VLM decision-clarity permutation recurrence requires explicit local configuration.",
    );
  ...
  const outputDir = join(
    process.cwd(),
    ".local-vlm",
    "decision-clarity-permutation-recurrence",
    "latest",
  );
  await mkdir(outputDir, { recursive: true });
  const files = await writeDecisionClarityPermutationRecurrenceDiagnosticFiles({
    report,
    outputDir,
    stem: "local-vlm-decision-clarity-permutation-recurrence",
  });
  console.log(`Wrote ${files.jsonPath}`);
  console.log(`Wrote ${files.markdownPath}`);
  console.log(`Recurrence classification: ${report.classification.recurrenceStatus}`);
}
