...
import {
  ...
  writeDecisionClarityReplicationDiagnosticFiles,
} from "../../src/fixtures/eval/vision-observer/local-vlm/decision-clarity-replication-diagnostic.ts";
...
async function main() {
  ...
  if (!config.ok) {
    console.log(
      "SKIP: local VLM decision-clarity replication requires explicit local configuration.",
    );
  ...
  const outputDir = join(process.cwd(), ".local-vlm", "decision-clarity-replication", "latest");
  await mkdir(outputDir, { recursive: true });
  const files = await writeDecisionClarityReplicationDiagnosticFiles({
    report,
    outputDir,
    stem: "local-vlm-decision-clarity-replication",
  });
  console.log(`Wrote ${files.jsonPath}`);
  console.log(`Wrote ${files.markdownPath}`);
  console.log(`Replication classification: ${report.classification.replicationEffect}`);
}
