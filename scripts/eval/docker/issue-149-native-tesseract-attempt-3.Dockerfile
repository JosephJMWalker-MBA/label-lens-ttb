# Issue #149 Attempt 3 — native Tesseract float-model compatibility probe.
#
...
# pinned base (discovery run 30474266684, reused unchanged for Attempt 3), not guessed. The runtime Leptonica
...
# between arms.
#
# Attempt 3 harness correction 1: the stock tessdata tree installed by the pinned
# package still ships Tesseract's `configs/tsv` renderer config. The probe copies
# that exact file into each ephemeral mount so the requested TSV renderer engages
# for both arms. Attempt 2 mounted bare model directories, so `configs/tsv` was
# absent and Tesseract silently fell back to plain text.
ENV LC_ALL=C \
    LANG=C \
    OMP_THREAD_LIMIT=1 \
    OMP_NUM_THREADS=1
