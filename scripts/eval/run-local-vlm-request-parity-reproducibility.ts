...
import {
  ...
  writeRequestParityReproducibilityDiagnosticFiles,
} from "../../src/fixtures/eval/vision-observer/local-vlm/request-parity-reproducibility-diagnostic.ts";
...
async function main() {
  ...
  if (!config.ok) {
    console.log(
      "SKIP: local VLM request parity reproducibility requires explicit local configuration.",
    );
  ...
  const outputDir = join(process.cwd(), ".local-vlm", "request-parity-reproducibility", "latest");
  await mkdir(outputDir, { recursive: true });
  const files = await writeRequestParityReproducibilityDiagnosticFiles({
    report,
    outputDir,
    stem: "local-vlm-request-parity-reproducibility",
  });
  console.log(`Wrote ${files.jsonPath}`);
  console.log(`Wrote ${files.markdownPath}`);
  console.log(`Overall classification: ${report.classification.overall}`);
}
