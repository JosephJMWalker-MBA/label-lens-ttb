...
7. **Panel Identity Persistence for Revision Seeds:** Submitted panel IDs are typed client identities from the package workstation, not bare UUIDs. MySQL stores those identities exactly in `submitted_panels.id`, while the application accepts only the shorter filesystem-safe subset that can form a bounded `{panelId}-{sha256}` storage filename. During seller revision seeding, `panelId` is the recovered canonical source identity used to remap seller evidence into a fresh browser-local draft. `assetPanelId` is only the legacy database row identity used by the authorized panel-byte route to fetch the stored asset. The server rejects any seed whose storage key, checksum suffix, namespace, or recovered identity cannot be reconciled exactly, including duplicate recovered `panelId` values. Historical canonical JSON, signatures, panel rows, and storage keys are never repaired, rewritten, or resigned to make recovery succeed.
