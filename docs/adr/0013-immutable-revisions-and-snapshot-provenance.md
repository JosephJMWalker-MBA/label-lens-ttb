...
We will implement an immutable, append-only revision model for all package submissions with HMAC integrity signatures:
...
4. **HMAC Integrity Signature:** The server computes an HMAC-SHA256 signature (`integrity_signature`) of the canonical JSON string, signed with the server-side environment secret key `LABEL_LENS_INTEGRITY_SECRET`. This signature is stored in the database and included on the seller's receipt.
5. **Validation on Load:** Whenever a revision snapshot is retrieved or audited, the server re-computes the HMAC-SHA256 using the environment secret and compares it against the database signature. If they do not match, the server logs a critical alert and aborts the request.
