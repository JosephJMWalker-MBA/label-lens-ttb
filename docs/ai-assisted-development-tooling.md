...
3. **Implementation agents — Codex, Opus, and Fable:** execute different classes of repository work at settings selected for the task.
...
| Claude Code — Fable | Current Fable mode; no separate effort setting recorded | Visual hierarchy, evidence presentation, responsive composition, and interaction-focused implementation built on approved architecture | Fable is used where visual and interaction quality dominate. It inherits established authority, evidence, accessibility, and API contracts rather than redefining them. |
...
- prepare exact task packets for Codex, Opus, and Fable;
...
This layer makes lower-effort and specialized tools more productive because they do not need to rediscover the entire product architecture. Each implementation agent receives:
...
The hard reasoning is performed once, preserved in conversation and GitHub, and converted into tasks that implementation agents can execute efficiently.
...
### 4.2 Codex 5.4 Extra High results
...
Fable receives visually concentrated tasks such as:
...
The coordination layer then checks the return report against GitHub before merge advice is given.
...
AI tools may inspect, reason, implement bounded changes, test, prepare PRs, and coordinate handoffs.
